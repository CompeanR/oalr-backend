...
describe('Authentication E2E (HTTP Layer)', () => {
    ...
    describe('GET /auth/validate', () => {
        it('should return 401 for missing authorization header', async () => {
            const response = await request(httpServer).get('/auth/validate').expect(401);
        ...
        it('should return 401 for malformed authorization header', async () => {
            // Missing "Bearer"
            await request(httpServer).get('/auth/validate').set('Authorization', 'some-token').expect(401);

            // Wrong scheme
            await request(httpServer).get('/auth/validate').set('Authorization', 'Basic some-token').expect(401);
        });
    });

    describe('GET /auth/google', () => {
        it('should initiate Google OAuth flow', async () => {
            const response = await request(httpServer).get('/auth/google').expect(302); // Redirect to Google
    ...
    describe('GET /auth/google/callback', () => {
        it('should handle OAuth callback endpoint', async () => {
            ...
            const response = await request(httpServer).get('/auth/google/callback').expect(401);
    ...
    describe('Authentication Endpoints Availability', () => {
        it('should have all auth endpoints available', async () => {
            ...
            const validateResponse = await request(httpServer).get('/auth/validate');
            expect(validateResponse.status).not.toBe(404);

            // GET /auth/google exists
            const googleResponse = await request(httpServer).get('/auth/google');
            expect(googleResponse.status).not.toBe(404);

            // GET /auth/google/callback exists
            const callbackResponse = await request(httpServer).get('/auth/google/callback');
    ...
    describe('HTTP Response Headers and Format', () => {
        ...
        it('should include proper CORS headers', async () => {
            const response = await request(httpServer).options('/auth/login');
        ...
        it('should handle invalid HTTP methods', async () => {
            ...
            const response = await request(httpServer).put('/auth/login').send({});
    ...
    describe('Rate Limiting (HTTP Level)', () => {
        it('should eventually apply rate limiting on login endpoint', async () => {
            ...
            const successfulOrUnauthorized = responses.filter(
                (r) => r.status === 401 || r.status === 200 || r.status === 500,
            );
    ...
    describe('Error Response Format Consistency', () => {
        it('should return consistent error format across endpoints', async () => {
            ...
            const loginError = await request(httpServer).post('/auth/login').send({ email: 'invalid' });
            ...
            const validateError = await request(httpServer).get('/auth/validate');
